...
(e.g. _acme-crm-1.0.4.Final.jar_) or retrieving modules from remote repositories using unique identifiers (e.g. _com.acme:acme-crm:1.0.4.Final_).
...
```
layrry-launcher-1.0.4-all.jar --layers-config <path/to/layers.yml> [program arguments]
```

E.g. like so:

```
layrry-launcher-1.0.4-all.jar --layers-config hello-world.yml Alice Bob
```
...
layers:
  log:
    modules:
      ...
      - "com.example:logconfig:1.0.4"
  foo:
    parents:
      - "log"
    modules:
      - "com.example:greeter:1.0.4"
      - "com.example:foo:1.0.4"
  bar:
    parents:
      - "log"
    modules:
      - "com.example:greeter:2.0.0"
      - "com.example:bar:1.0.4"
  app:
    parents:
      - "foo"
      - "bar"
    modules:
      - "com.example:app:1.0.4"
main:
...
[layers.log]
  modules = [
    "org.apache.logging.log4j:log4j-api:jar:2.13.1",
    "org.apache.logging.log4j:log4j-core:jar:2.13.1",
    "com.example.it:it-logconfig:1.0.4"]
[layers.foo]
  parents = ["log"]
  modules = [
    "com.example.it:it-greeter:1.0.4",
    "com.example.it:it-foo:1.0.4"]
[layers.bar]
  parents = ["log"]
  modules = [
    "com.example.it:it-greeter:2.0.0",
    "com.example.it:it-bar:1.0.4"]
[layers.app]
  parents = ["foo", "bar"]
  modules = ["com.example.it:it-app:1.0.4"]
[main]
  module = "com.example.app"
  class = "com.example.app.App"
```
...
javafx_version = 11.0.4
tilesfx_version = 11.44
layrry_version = 1.0.4
```

This application can be launched as

```
layrry-launcher-1.0.4-all.jar --layers-config layers.toml --properties versions.properties
```
...
Layers layers = Layers.builder()
    .layer("log")
        ...
        .withModule("com.example:logconfig:1.0.4")
    .layer("foo")
        .withParent("log")
        .withModule("com.example:greeter:1.0.4")
        .withModule("com.example:foo:1.0.4")
    .layer("bar")
        .withParent("log")
        .withModule("com.example:greeter:2.0.0")
        .withModule("com.example:bar:1.0.4")
    .layer("app")
        .withParent("foo")
        .withParent("bar")
        .withModule("com.example:app:1.0.4")
    .build();
