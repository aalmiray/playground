...
(e.g. _acme-crm-1.0.6.Final.jar_) or retrieving modules from remote repositories using unique identifiers (e.g. _com.acme:acme-crm:1.0.6.Final_).
...
```
layrry-launcher-1.0.6-all.jar --layers-config <path/to/layers.yml> [program arguments]
```

E.g. like so:

```
layrry-launcher-1.0.6-all.jar --layers-config hello-world.yml Alice Bob
```
...
layers:
  log:
    modules:
      ...
      - "com.example:logconfig:1.0.6"
  foo:
    parents:
      - "log"
    modules:
      - "com.example:greeter:1.0.6"
      - "com.example:foo:1.0.6"
  bar:
    parents:
      - "log"
    modules:
      - "com.example:greeter:2.0.0"
      - "com.example:bar:1.0.6"
  app:
    parents:
      - "foo"
      - "bar"
    modules:
      - "com.example:app:1.0.6"
main:
...
[layers.log]
  modules = [
    "org.apache.logging.log4j:log4j-api:jar:2.13.1",
    "org.apache.logging.log4j:log4j-core:jar:2.13.1",
    "com.example.it:it-logconfig:1.0.6"]
[layers.foo]
  parents = ["log"]
  modules = [
    "com.example.it:it-greeter:1.0.6",
    "com.example.it:it-foo:1.0.6"]
[layers.bar]
  parents = ["log"]
  modules = [
    "com.example.it:it-greeter:2.0.0",
    "com.example.it:it-bar:1.0.6"]
[layers.app]
  parents = ["foo", "bar"]
  modules = ["com.example.it:it-app:1.0.6"]
[main]
  module = "com.example.app"
  class = "com.example.app.App"
```
...
javafx_version = 11.0.6
tilesfx_version = 11.44
layrry_version = 1.0.6
```

This application can be launched as

```
layrry-launcher-1.0.6-all.jar --layers-config layers.toml --properties versions.properties
```
...
Layers layers = Layers.builder()
    .layer("log")
        ...
        .withModule("com.example:logconfig:1.0.6")
    .layer("foo")
        .withParent("log")
        .withModule("com.example:greeter:1.0.6")
        .withModule("com.example:foo:1.0.6")
    .layer("bar")
        .withParent("log")
        .withModule("com.example:greeter:2.0.0")
        .withModule("com.example:bar:1.0.6")
    .layer("app")
        .withParent("foo")
        .withParent("bar")
        .withModule("com.example:app:1.0.6")
    .build();
